...
export default function Login({ onLogin }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (isLogin) {
      ...
      const newUser = { id: Date.now(), name: name||'RMBD User', email: key, password, avatar: avatar || '' };
  ...
  return (
    <div className="min-h-screen w-full bg-[#e89e9e] flex flex-col font-serif overflow-hidden">
      <div className="flex-1 flex flex-col lg:flex-row px-4 md:px-8 gap-6 items-center justify-center py-6">
        ...
        <div className="flex-1 max-w- w-full">
          <div className="bg-[#fff6f0] rounded- p-5 shadow-xl">
            ...
            {avatar && <img src={avatar} className="w-16 h-16 rounded-full mx-auto mb-3 object-cover" />}
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <label className="block text- bg-[#f5e6d8] px-3 py-2 rounded-full text-center cursor-pointer">📷 Profile Photo <input type="file" accept="image/*" onChange={handleAvatar} className="hidden" /></label></>}
            ...
            {msg && <div className="mt-3 text- p-2 rounded-xl text-center bg-red-100">{msg}</div>}
          </div>
        </div>
      </div>
    </div>
