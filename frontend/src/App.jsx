...
export default function App(){
  ...
  useEffect(()=>{
    ...
    onDisconnect(myPresenceRef).set({ online: false, name: currentUser.name, email: currentUser.email, avatar: currentUser.avatar||'', lastSeen: Date.now() });
      ...
      <div className="max-w-[1200px] mx-auto flex gap-4 p-3">
        <div className="hidden md:block w-[250px] shrink-0">
          <div className="bg-white rounded-xl p-3 shadow-sm">
            <p className="font-bold text-[13px] mb-3">Menu - {Object.values(presences).filter(p=>p.online).length} Online</p>
            <div className="space-y-1 text-[13px]">
              <div className="bg-blue-50 text-[#0866ff] p-2.5 rounded-lg font-bold">🏠 Timeline</div>
              <div onClick={()=>setShowMessenger(true)} className="p-2.5 hover:bg-gray-100 rounded-lg cursor-pointer">💬 Messenger ({contacts.filter(c=>getPresenceStatus(c.email).online).length} online)</div>
              <div className="p-2.5 hover:bg-gray-100 rounded-lg cursor-pointer">👥 Friends ({Object.keys(friends).length})</div>
            </div>
          </div>
        </div>
        ...
        <div className="hidden lg:block w-[320px] shrink-0">
          <div className="bg-white rounded-xl p-3 shadow-sm sticky top-[70px]">
            ...
            {contacts.length===0?<p className="text-[11px] text-gray-500">No contacts yet</p>:contacts.map(u=>{
            ...
            <div className="mt-3 p-2 bg-gray-50 rounded-lg">
              <p className="text-[10px] font-bold">Legend:</p>
              <p className="text-[10px]"><span className="inline-block w-2 h-2 bg-green-500 rounded-full mr-1"></span> Green = Online / Active now</p>
              <p className="text-[10px]"><span className="inline-block w-2 h-2 bg-gray-400 rounded-full mr-1"></span> Gray = Offline / Active 5m ago etc</p>
            </div>
          </div>
        </div>
      </div>
